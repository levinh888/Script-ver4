...
-- Times Attack Pro – Đánh cực nhanh không lỗi nhảy
AddToggle(Tab3, {
...
-- TAB 4: Speed 🚀
local Tab4 = MakeTab({ Name = "Speed 🚀" })

AddToggle(Tab4, {
    Name = "⚡ Bật Tốc Độ Chạy",
    Callback = function(state)
        _G.speedOn = state
        if state then
            task.spawn(function()
...
AddButton(Tab4, {
    Name = "↩️ Reset Tốc Độ Về Mặc Định",
    Callback = function()
        local humanoid = Character:FindFirstChildOfClass("Humanoid")
        if humanoid then
            humanoid.WalkSpeed = 16
...
-- TAB 5: Jump / Fly 🌈
local Tab5 = MakeTab({ Name = "Jump / Fly 🌈" })
